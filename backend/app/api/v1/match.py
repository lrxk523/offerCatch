...
async def match_resume_pdf(
    ...
    try:
            ...
            try:
                ...
                if jd_contents:
                    ...
                    if jd_ocr_text and jd_ocr_text.strip():
                        ...
                        print(f"[Match-PDF] JD 图片 OCR: {len(final_jd_text)} 字符")
            except Exception as e:
                print(f"[Match-PDF] JD 图片 OCR 失败: {e}")
        ...
        print(f"[Match-PDF] PDF 共 {total_pages} 页，开始逐页 OCR...")
        ...
        print(f"[Match-PDF] OCR 完成，共 {len(ocr_full_text)} 字符")
